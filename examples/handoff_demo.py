...
async def connect(stack, path, agent):
    ...
    env = {"AGENT_MEMORY_PATH": str(path), "AGENT_MEMORY_AGENT": agent,
...
async def demo():
    with tempfile.TemporaryDirectory(prefix="memory-handoff-") as directory:
        async with AsyncExitStack() as stack:
            ...
            saved = await call(first, "memory_write", text="Queue delivery retries at most three times.",
                               type="decision", source={"path": "queue/policy.py", "event": "code_review"})
            entry_id = saved.split()[1]
            await call(first, "memory_handoff", done="Reviewed queue delivery retries.", next_steps="Check the retry boundary.")
            context = await call(second, "memory_boot", task="Check queue delivery retry boundary.")
            assert "three times" in context and "agent-a" in context
            print("1. Agent B receives Agent A's decision and handoff over MCP.")

            await call(second, "memory_update", id=entry_id,
                       text="Queue delivery retries at most five times.", expected_revision=1)
            conflict = await call(first, "memory_forget", id=entry_id, expected_revision=1, error=True)
            assert "revision" in conflict and "current 2" in conflict
            print("2. Agent A's stale deletion is rejected: expected revision 1, current 2.")

            saved = await call(first, "memory_supersede", id=entry_id,
                               text="Queue delivery retries at most twice; failed jobs go to dead letter.", expected_revision=2)
            ...
            recalled = await call(second, "memory_recall", query="Queue delivery retries", budget_tokens=300)
            assert "twice" in recalled and "five times" not in recalled and "three times" not in recalled
            print("3. A replacement decision is recalled; the old decision and its revisions remain inspectable.")
            print("All checks passed. Two server processes; one temporary store; no model calls.")
