...
def test_runner_runs_matched_arms_and_retains_missing_telemetry(tmp_path, monkeypatch):
    ...
    report = evaluate_tasks([sys.executable, str(wrapper)], output=tmp_path / "results", label="test-double",
                            split="calibration", repetitions=1, timeout=10)
    assert all(row["passed"] == row["runs"] == 1 for row in report["summary"].values())
    assert all(row["mean_full_session_tokens"] is None for row in report["summary"].values())
    rows = [json.loads(line) for line in (tmp_path / "results" / "runs.jsonl").read_text().splitlines()]
    assert {row["arm"] for row in rows} == {"engine", "curated_markdown", "no_memory"}
    assert len({row["task"] for row in rows}) == 1
    with pytest.raises(FileExistsError):
        evaluate_tasks([sys.executable, str(wrapper)], output=tmp_path / "results", label="test-double")


def test_agent_timeout_is_bounded(tmp_path):
    with pytest.raises(TimeoutError):
        run_agent([sys.executable, "-c", "import time; time.sleep(60)"], {"workspace": str(tmp_path)}, timeout=0.1)


def test_errors_count_as_failed_runs_and_missing_usage_stays_unknown():
    rows = [{"arm": "engine", "passed": False, "error": "timeout", "usage": None,
             "seconds": 10, "memory_tokens": 100, "scenario": "fresh"},
            {"arm": "engine", "passed": True, "error": None, "usage": {"input_tokens": 200, "output_tokens": 100},
             "seconds": 5, "memory_tokens": 100, "scenario": "fresh"}]
...
def test_mcp_demo_uses_two_stdio_processes():
    ...
    result = subprocess.run([sys.executable, str(path)], capture_output=True, text=True, timeout=40)
